...
def generate_positions(n_nodes: int = 30, length: float = 1.0, jitter: float = 0.01, seed: int = 7) -> np.ndarray:
...
def build_radius_adjacency(positions: np.ndarray, radius: float = 0.09) -> np.ndarray:
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if abs(positions[i] - positions[j]) <= radius:
                A[i, j] = 1.0
                A[j, i] = 1.0
...
def corrected_pattern(n_nodes: int, corrected_fraction: float = 0.1, mode: str = "left_cluster", seed: int = 7) -> np.ndarray:
